...
struct PSpage {
    ...
    PSpage(const char *filnam) {
        ...
        setfont(const_cast<char *>("Times-Roman"), 12.);
        setlinewidth(0.5);
    }
    ...
    void setdash(const char *patt, Int phase = 0) {
    ...
    void addtext(const char *text) { fprintf(PLT, "(%s) show ", text); }

    void puttext(const char *text, Doub x, Doub y, Doub rot = 0.0) {
        fprintf(PLT, "gsave %g %g translate %g rotate 0 0 mt ", x, y, rot);
        addtext(text);
        fprintf(PLT, "grestore \n");
    }

    void putctext(const char *text, Doub x, Doub y, Doub rot = 0.0) {
...
struct PSplot : PSpage {
    ...
    void clip() {
        gsave();
        polyline(xbox, ybox, 1, 0, 1);
    }

    void clip(VecDoub &x, VecDoub &y) {
        gsave();
        polyline(x, y, 1, 0, 1);
    }
    ...
    void xlabel(const char *text) { putctext(text, 0.5 * (pll + pur), qll - 2. * fontsize - 8.); }

    void ylabel(const char *text) { putctext(text, pll - 3. * fontsize - 8., 0.5 * (qll + qur), 90.); }

    void label(const char *text, double x, double y, double rot = 0.) { puttext(text, p(x), q(y), rot); }
